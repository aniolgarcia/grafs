...
amb $d(v)$. La successió de graus d'un graf serà la successió que
...
$G$ queda determinat de la següent manera: $\delta(G)=min\{d(g):v\text{\ensuremath{\in}}V(G)\}$.
De manera similar, el grau màxim de $G$, $\text{\ensuremath{\Delta}}(G)=max\{d(v):v\text{\ensuremath{\in}}V\}$.
...
nombre d'arestes.''\[ \sum_{v\in V}d(v)=2|E| \]
\end{quotation}
...
que:\[ \sum_{v\in V(G)}d(v)=\sum_{v\in V(H)}d(v)+2=2(|E|-1)+2=2|E| \]D'aquesta
...
Els grafs xarxes bidimensionsals $G_{m,n}$ són grafs bipartits que formen una xarxa de $m*n$ vèrtexs. Un graf xarxa té $mn$ vèrtexs i $(m-1)n+(n-1)m$. Es pot greneralitzar per a xarxes de més dimensions com a $G_{m,n,o,...}$.
...
\\
Hi ha un tipus especial d'arbres anomenats elementals, que són els arbres amb $|V|=1$, $|V|=2$ i en general tots aquells on $\delta=1$ i $\Delta=2$. S'anomenen $P_{n}$, on $n=|V|$. També es pot pensar en grafs elementals com a $G_{n,1}$.(adjuntar exemples de grafs elementals) 
...
\end{quote}
\emph{Demostració: } Com a conseqüència del Teorema 1 i Teorema 2 podem dir que en un arbre \[\sum_{v\in V}d(v)=2|E|=2|V|-2\] També podem deduïr que si tots els vèrtexs tinguessin un grau $d(v_{i}) > 1$, llavors \[ \sum_{v\in V}d(v) \geq \sum_{v\in V}2 =2|V|\] Sabem que això no és correcte, ja que estem dient que $2|V|-2\geq2|V|$. Amb això ja podem veure que necessitem treure com a mínim dos graus, però demostrem també que amb un sol node de grau 1 tampoc és suficient. \smallskip Suposem ara que l'arbre té un sol node tal que $d(v)=1$ i elsaltres tenencom a mínim grau 2. Llavors \[ 2|V|-2=\sum_{v\in V}d(v)=1+\sum_{\substack{v\in V\\\text{si }d(v)\neq 1}}d(v)\geq 1+\sum_{\substack{v\in V \\\text{si }d(v)\neq 1}}2=1+2(|V|-1)=2|V|-1 \] Ara estem dient que $2|V|-2 \geq 2|V|-1$, que torna a ser una contradicció. Sabem que si treiem un grau més, el teorema es complirà, i hem demostrat que el mínim nombre de vèrtexs de grau 1 és 2.
...
\end{quote}
\emph{Demostració: }Si eliminem el node de grau $\Delta$ de l'arbre, juntament amb totes les seves arestes incidents, obtenim un conjunt de $\Delta$ grafs. Si alguns d'aquests grafs consisteixen en tan sols un node, vol dir que abans eren adjacents al node que hem eliminat, per tant, només tenien grau 1. Si, pel contrari, formen nous arbres, pel teorema 3 podem dir que hi haurà com a mínim dues fulles.Encara que hi ha la possibilitat que un dels nodes amb grau 1 sigués l'adjacent a el node que hem tret, sempre podem garantir que hi ha com a mínim una fulla. Per tant, també podem garantir que que hi hauràn com a mínim $\Delta$ fulles.
