...
Aquest métode, però, té un problema, i és que només funciona per a grafs no dirigits. Hi ha la possibilitat que, treballant amb un graf dirigit, un node del graf no sigui accessible des del node inicial que hem determinat. Aixó es pot arreglar executant l'algorisme fent que cada node no visitat per les iteracions anteriors sigui l'inicial. El segon algorisme, a part d'arreglar això, utilitza una funció recursiva (una funció que es crida a si mateixa).

\begin{algorisme}
\KwData{Un graf $G$}
\KwResult{Seqüència de nodes visitats des de cada node}
nova llista $anterior$\;
\Fn{DFS_recursiu($G, v$)}
{
	\ForEach{node $s$ adjacent a $v$}
	{	
		\If{$s$ no existeix dins $anterior$}
		{
			$anterior[s]=v$\;
			DFS_recursiu($Adj, s$)\;		
		}
