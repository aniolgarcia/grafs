...
Per indicar el complementari de $G$ s'escriu $\bar{G}$ o $G'$. Per obtenir el graf complementari
...
truncats dobles}; els $8$-regulars, \textit{grafs de 24 cel·les}... Evidentment,
...
On:
\begin{description}
\item[$\frac{1}{2}$],ja que contem dues vegades el cicle, ja que no són dirigits.
...
senar. 
Llavors, podem deduïr que tots els grafs $C_{n}$ amb $n$
...
$W$. 
S'anomenen $K_{m,n}$, on $m=|U|$ i $n=|W|$.
...
S'anomenen $S_{n}$ i són estrelles tots els grafs bipartits de la forma $K_{1,n-1}$ o $K_{n-1,1}$. 
...
El nombre de cicles que conté un graf amb $n$ vètrexs està determinat per $n^{2}-3n+3$.
\\
